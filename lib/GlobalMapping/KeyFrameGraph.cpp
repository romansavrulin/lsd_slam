...
KeyFrameGraph::KeyFrameGraph() : nextEdgeId(0) {
  ...
  BlockSolver *blockSolver =
      new BlockSolver(std::unique_ptr<LinearSolver>(solver));
...
void KeyFrameGraph::dumpMap(std::string folder) {
  ...
  // 	cv::imwrite(buf, cv::Mat(keyframesAll[i]->height(),
  // keyframesAll[i]->width(),CV_32F,keyframesAll[i]->image()));
  //
  // 	snprintf(buf, 100, "%s/var-%d.png", folder.c_str(), i);
  // 	cv::imwrite(buf,
  // getVarRedGreenPlot(keyframesAll[i]->idepthVar(),keyframesAll[i]->image(),keyframesAll[i]->width(),keyframesAll[i]->height()));
  // }
  //
  //
  ...
  // e->meanResidual; 	resD(e->firstFrame->idxInKeyframes,
  // e->secondFrame->idxInKeyframes) = e->meanResidualD;
  // 	resP(e->firstFrame->idxInKeyframes, e->secondFrame->idxInKeyframes) =
  // e->meanResidualP; 	usage(e->firstFrame->idxInKeyframes,
  // e->secondFrame->idxInKeyframes) = e->usage;
...
bool KeyFrameGraph::addElementsFromBuffer() {
  ...
  for (auto newKF : newKeyframesBuffer) {
    ...
    CHECK(!newKF->frame()->pose->isInGraph)
        << "Keyframe" << newKF->id()
        << " was already in the graph when attempted to add it";
...
void KeyFrameGraph::calculateGraphDistancesToFrame(
    const KeyFrame::SharedPtr &startFrame,
    std::unordered_map<KeyFrame::SharedPtr, int> &distanceMap) {

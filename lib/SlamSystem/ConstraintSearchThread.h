...
public:
  ...
  void doFullReConstraintSearch(void) {
    ...
    if (_thread)
      _thread->send(
          std::bind(&ConstraintSearchThread::fullReconstraintSearchImpl, this));
  }

  // Note in non-threaded mode, this does nothing!
  void doCheckNewKeyFrame(const KeyFrame::SharedPtr &keyframe) {
    if (_thread)
      _thread->send(std::bind(&ConstraintSearchThread::checkNewKeyFrameImpl,
                              this, keyframe));
  }
...
private:
  ...
  //=== Internal functions ====
  int findConstraintsForNewKeyFrames(const KeyFrame::SharedPtr &newKeyFrame,
                                     bool forceParent, bool useFABMAP,
                                     float closeCandidatesTH);

  std::unique_ptr<active_object::ActiveIdle> _thread;

  float tryTrackSim3(const KeyFrame::SharedPtr &kfA,
                     const KeyFrame::SharedPtr &kfB, int lvlStart, int lvlEnd,
                     bool useSSE, Sim3 &AtoB, Sim3 &BtoA,
                     KFConstraintStruct *e1 = nullptr,
                     KFConstraintStruct *e2 = nullptr);

  bool testConstraint(const KeyFrame::SharedPtr &keyframe,

...
MappingThread::MappingThread(SlamSystem &system)
    : unmappedTrackedFrames(), unmappedTrackedFramesMutex(),
      trackedFramesMapped(), relocalizer(),
      mappingTrackingReference(new TrackingReference()), _system(system),
      _newKeyFrame(nullptr), _newImageSet(nullptr),
...
void MappingThread::callbackUnmappedTrackedFrames( void )
{
        ...
        size_t sz = 0;
        {
                ...
                if( sz > 0 )
                        nMapped =
unmappedTrackedFrames.back()->trackingParent()->numMappedOnThisTotal < 10;
        }

        LOG(INFO) << "In unmapped tracked frames callback with " << sz << "
frames";
        ...
        if(sz < 50 ||
          (sz < 100 && nMapped) ) {
                ...
                // TODO:  Was originally in the while() loop above.  However,
there are
                // circumstances (if there's one untracked thread in the queue
referenced
                // to an older keyframe) where doMappingIteration will return
false, and this
                // notify never happens.   If that happens, TrackingThread will
stop if it's
                // waiting on the signal.
                //
                // This should be called once per callback otherwise
TrackingThread might get hung up?
...
void MappingThread::callbackUnmappedTrackedSet(void) {
  ...
  size_t sz = 0;
  {
    ...
    if (sz > 0)
      nMapped = unmappedTrackedSets.back()
                    ->refFrame()
                    ->trackingParent()
                    ->numMappedOnThisTotal < 10;
  }

  LOG(INFO) << "In unmapped tracked frames callback with " << sz << " frames";

  if (sz < 50 || (sz < 100 && nMapped)) {
    ...
    // to an older keyframe) where doMappingIteration will return false, and
    // this notify never happens.   If that happens, TrackingThread will stop if
    // it's waiting on the signal.
    //
    // This should be called once per callback otherwise TrackingThread might
    // get hung up?
...
bool MappingThread::doMappingIteration()
{
        ...
        //
        // 	map->invalidate();
        // 	LOGF(INFO, "Finished KF %d as Mapping got
disabled!\n",currentKeyFrame()->id());
        //
...
lastTrackingClosenessScore);
                // 	if(_system.trackingThread->lastTrackingClosenessScore >
1)
                // 		changeKeyframe(true, false,
_system.trackingThread->lastTrackingClosenessScore * 0.75);
                //
                ...
                if( frame ) {
                        LOG(INFO) << "Set " << frame->id() << " as new key
frame"; finishCurrentKeyframe(); _system.changeKeyframe(frame, false,
true, 1.0f);
...
(didSomething ? "DID" : "DIDN'T") << " do something";
        }
        ...
        else
        {
                ...
                {
                        if( _system.currentKeyFrame()->numMappedOnThisTotal >=
MIN_NUM_MAPPED) finishCurrentKeyframe(); else discardCurrentKeyframe();
...
bool MappingThread::doMappingIterationSet() {
  ...
  if (_system.trackingThread->trackingIsGood()) {
    ...
    LOG(DEBUG) << "Tracking is good, updating image set frame, "
               << (didSomething ? "DID" : "DIDN'T") << " do something";
  }
...
) ) { if( unmappedTrackedFrames.front()->hasTrackingParent() ) { LOG(INFO) <<
"Dropping frame " << unmappedTrackedFrames.front()->id()
                                                                        << " its
has tracking parent " << unmappedTrackedFrames.front()->trackingParent()->id()
                                                                                << " current keyframe is " << _system.currentKeyFrame()->id();
                                                } else {
                                                        LOG(INFO) << "Dropping
frame " << unmappedTrackedFrames.front()->id() << " which doesn't have a
tracking parent";
                                                }
        ...
        if(unmappedTrackedFrames.size() > 0) {
                ...
                LOGF_IF( INFO, Conf().print.threadingInfo,
                        "MAPPING frames %d to %d (%d frames) onto keyframe %d",
...
bool MappingThread::updateImageSet() {
  ...
  while (unmappedTrackedSets.size() > 0 &&
         (!unmappedTrackedSets.front()->refFrame()->hasTrackingParent() ||
          !unmappedTrackedSets.front()->refFrame()->isTrackingParent(
              _system.currentKeyFrame()))) {
    if (unmappedTrackedSets.front()->refFrame()->hasTrackingParent()) {
      LOG(INFO)
          << "Dropping frame " << unmappedTrackedSets.front()->refFrame()->id()
          << " its has tracking parent "
          << unmappedTrackedSets.front()->refFrame()->trackingParent()->id()
          << " current keyframe is " << _system.currentKeyFrame()->id();
    } else {
      LOG(INFO) << "Dropping frame " << unmappedTrackedSets.front()->id()
                << " which doesn't have a tracking parent";
    }
  ...
  if (unmappedTrackedSets.size() > 0) {
    ...
    LOGF_IF(INFO, Conf().print.threadingInfo,
            ...
            (int)references.size(), _system.currentKeyFrame()->id());
...
void MappingThread::finishCurrentKeyframe() {
  LOG_IF(DEBUG, Conf().print.threadingInfo)
      << "FINALIZING KF " << _system.currentKeyFrame()->id();
  ...
  if (Conf().SLAMEnabled) {
    ...
    if (_system.currentKeyFrame()->idxInKeyframes < 0) {
      ...
      _system.currentKeyFrame()->idxInKeyframes =
          _system.keyFrameGraph()->keyframesAll.size();
      _system.keyFrameGraph()->keyframesAll.push_back(
          _system.currentKeyFrame());
      _system.keyFrameGraph()->totalPoints +=
          _system.currentKeyFrame()->numPoints;
  ...
  LOG(DEBUG) << "Finishing current keyframe, publishing keyframe "
             << _system.currentKeyFrame()->id();
  _system.publishCurrentKeyframe();
  _system.publishKeyframeGraph();
  //_system.publishPointCloud();
...
void MappingThread::discardCurrentKeyframe() {
  LOG_IF(DEBUG, Conf().print.threadingInfo)
      << "DISCARDING KF " << _system.currentKeyFrame()->id();

  if (_system.currentKeyFrame()->idxInKeyframes >= 0) {
    LOG(WARNING) << "WARNING: trying to discard a KF that has already been "
                    "added to the graph... finalizing instead.";
    finishCurrentKeyframe();
    return;
  }

...
void KeyFrameGraphDisplay::draw()
{
	...
	if(flushPointcloud)
	{
		...
		printf("Flushing Pointcloud to %s!\n", (ros::package::getPath("lsd_slam_core")+"/pc_tmp.ply").c_str());
		std::ofstream f((ros::package::getPath("lsd_slam_core")+"/pc_tmp.ply").c_str());
		int numpts = 0;
		for(unsigned int i=0;i<keyframes.size();i++)
		{
		...
		std::ofstream f2((ros::package::getPath("lsd_slam_core")+"/pc.ply").c_str());
		f2 << std::string("ply\n");
		...
		std::ifstream f3((ros::package::getPath("lsd_slam_core")+"/pc_tmp.ply").c_str());
		while(!f3.eof()) f2.put(f3.get());

		f2.close();
		f3.close();

		system(("rm "+ros::package::getPath("lsd_slam_core")+"/pc_tmp.ply").c_str());
		flushPointcloud = false;
...
void KeyFrameGraphDisplay::addMsg(lsd_slam_core::keyframeMsgConstPtr msg)
{
	dataMutex.lock();
...
void KeyFrameGraphDisplay::addGraphMsg(lsd_slam_core::keyframeGraphMsgConstPtr msg)
{
	dataMutex.lock();

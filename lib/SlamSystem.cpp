...
SlamSystem::SlamSystem()
    : _perf(), _outputWrapper(new NullOutput3DWrapper()), _finalized(),
      _initialized(false), _keyFrameGraph(new KeyFrameGraph),
...
void SlamSystem::initialize(const std::shared_ptr<ImageSet> &set) {
  LOG_IF(FATAL, !Conf().doMapping)
      << "WARNING: mapping is disabled, but we just initialized... THIS WILL "
         "NOT WORK! Set doMapping to true.";
...
void SlamSystem::nextImageSet(const std::shared_ptr<ImageSet> &set) {
  ...
  // LOG(INFO) << "Tracking frame; " << ( blockUntilMapped ? "WILL" : "won't")
  // << " block";
...
void SlamSystem::changeKeyframe(const Frame::SharedPtr &candidate,
                                bool noCreate, bool force, float maxScore) {
  ...
  if (newReferenceKF != 0) {
    ...
    if (force) {
      if (noCreate) {
        LOG(INFO) << "mapping is disabled & moved outside of known map. "
                     "Starting Relocalizer!";
...
void SlamSystem::loadNewCurrentKeyframe(
    const Frame::SharedPtr &keyframeToLoad) {
  depthMap()->activateExistingKF(keyframeToLoad);

  LOG_IF(DEBUG, Conf().print.regularizeStatistics)
      << "re-activate frame " << keyframeToLoad->id() << "!";
...
void SlamSystem::createNewCurrentKeyframe(
    const Frame::SharedPtr &newKeyframeCandidate) {
  LOG_IF(INFO, Conf().print.threadingInfo)
      << "CREATE NEW KF " << newKeyframeCandidate->id() << ", replacing "
      << currentKeyFrame()->id();
  ...
  if (Conf().SLAMEnabled) {
    ...
    keyFrameGraph()->idToKeyFrame.insert(
        std::make_pair(newKeyframeCandidate->id(), newKeyframeCandidate));
  }
...
void SlamSystem::logPerformanceData() {
  ...
  if (sPassed > 1.0f) {
    ...
    LOGF(DEBUG,
         "Mapping: %3.1fms (%.1fHz); Track: %3.1fms (%.1fHz); Create: %3.1fms "
         "(%.1fHz); FindRef: %3.1fms (%.1fHz); PermaTrk: %3.1fms (%.1fHz); "
         ...
         depthMap()->perf().create.rate(), _perf.findReferences.ms(),
         _perf.findReferences.rate(), 0.0, 0.0,
         // trackableKeyFrameSearch != 0 ?
         // trackableKeyFrameSearch->trackPermaRef.ms() : 0,
         // trackableKeyFrameSearch != 0 ?
         // trackableKeyFrameSearch->trackPermaRef.rate() : 0,
         optThread->perf.ms(), optThread->perf.rate(),
         constraintThread->perf().findConstraint.ms(),
         constraintThread->perf().findConstraint.rate());
...
void SlamSystem::updateDisplayDepthMap() {
  ...
  if (Conf().onSceenInfoDisplay) {
    snprintf(
        buf1, 200, "Map: Upd %3.0fms (%2.0fHz); Trk %3.0fms (%2.0fHz); %d / %d",
        depthMap()->perf().update.ms(), depthMap()->perf().update.rate(),
        trackingThread->perf().track.ms(), trackingThread->perf().track.rate(),
        currentKeyFrame()->numFramesTrackedOnThis,
        currentKeyFrame()
            ->numMappedOnThis); //, (int)unmappedTrackedFrames().size());
...
Sophus::SE3d SlamSystem::getCurrentPoseEstimate() {
  boost::shared_lock_guard<boost::shared_mutex> lock(
      keyFrameGraph()->allFramePosesMutex);
...
void SlamSystem::publishPointCloud() {
  if (_outputWrapper)
    _outputWrapper->publishPointCloud(currentKeyFrame());
}

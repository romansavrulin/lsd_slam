...
DepthMap::DepthMap()
    : _perf(), _debugImages(Conf().slamImageSize), activeKeyFrame(nullptr),
      oldest_referenceFrame(nullptr), newest_referenceFrame(nullptr) {
...
void DepthMap::updateKeyframe(std::deque<Frame::SharedPtr> referenceFrames) {
  ...
  for (std::shared_ptr<Frame> frame : referenceFrames) {
    ...
    if (!frame->isTrackingParent(activeKeyFrame)) {
      LOGF(WARNING,
           "updating frame %d with %d, which was tracked on a different frame "
           "(%d).  While this should work, it is not recommended.",
           activeKeyFrame->id(), frame->id(), frame->trackingParent()->id());
    }
    ...
    if (frame->trackingParent()->id() == activeKeyFrame->id())
      ...
      refToKf =
          activeKeyFrame->getCamToWorld().inverse() * frame->getCamToWorld();
  ...
  if (plotStereoImages)
    _debugImages.plotUpdateKeyFrame(activeKeyFrame, oldest_referenceFrame,
...
void DepthMap::createKeyFrame(const Frame::SharedPtr &new_keyframe) {
  ...
  if (plotStereoImages)
    _debugImages.plotNewKeyFrame(new_keyframe);
  ...
  activeKeyFrame->pose->thisToParent_raw =
      sim3FromSE3(oldToNew_SE3.inverse(), rescaleFactor);
    ...
    for (int x = 0; x < Conf().slamImageSize.width; x++) {
      ...
      if (*idepthVar > 0) {
        *pt = DepthMapPixelHypothesis(*idepth, *idepthVar, *validity,
        ...
        currentDepthMap[idx].blacklisted =
            (*idepthVar == -2) ? MIN_BLACKLIST - 1 : 0;
      }
  ...
  */
  ...
  for (int y = 0; y < Conf().slamImageSize.height; y++) {
    for (int x = 0; x < Conf().slamImageSize.width; x++) {
      ...
      if (!isnanf(idepthValue) && idepthValue > 0) {
        currentDepthMap[idx] = DepthMapPixelHypothesis(
            idepthValue, idepthValue, VAR_GT_INIT_INITIAL, VAR_GT_INIT_INITIAL,
            20, Conf().debugDisplay);
      } else {
        currentDepthMap[idx].isValid = false;
        currentDepthMap[idx].blacklisted = 0;
...
void DepthMap::observeDepth() {
  ...
  LOGF_IF(DEBUG, Conf().print.observeStatistics,
          ...
          "init-blacklisted",
          activeKeyFrame->id(), runningStats.num_observe_created,
  ...
  LOGF_IF(DEBUG, Conf().print.observePurgeStatistics,
          ...
          "failed: %d; addSkip: %d;",
          activeKeyFrame->id(), runningStats.num_observe_good,
...
bool DepthMap::observeDepthCreate(const int &x, const int &y, const int &idx,
  ...
  Frame::SharedPtr refFrame(activeKeyFrameIsReactivated
                                ? newest_referenceFrame
  ...
  float error = doLineStereo(
      new_u, new_v, epx, epy, 0.0f, 1.0f, 1.0f / MIN_DEPTH, refFrame.get(),
      refFrame->image(0), result_idepth, result_var, result_eplLength, stats);
...
bool DepthMap::observeDepthUpdate(const int &x, const int &y, const int &idx,
  ...
  if (!activeKeyFrameIsReactivated) {
    if ((int)target->nextStereoFrameMinID - referenceFrameByID_offset >=
      ...
      if (plotStereoImages)
        _debugImages.setHypothesisHandling(
            x, y, cv::Vec3b(0, 255, 0)); // GREEN FOR skip
  ...
  float error =
      doLineStereo(x, y, epx, epy, min_idepth, target->idepth_smoothed,
                   max_idepth, refFrame.get(), refFrame->image(0),
...
void DepthMap::propagateDepth(const Frame::SharedPtr &new_keyframe) {
  ...
  LOGF_IF(WARNING, (!new_keyframe->isTrackingParent(activeKeyFrame)),
          ...
          "which was tracked on a different frame (%d).  While this should "
  ...
  const bool *trackingWasGood = (new_keyframe->isTrackingParent(activeKeyFrame)
                                     ? new_keyframe->refPixelWasGoodNoCreate()
                                     : nullptr);
  ...
  const float fx = new_keyframe->fx(), fy = new_keyframe->fy(),
              cx = new_keyframe->cx(), cy = new_keyframe->cy(),
              fxi = new_keyframe->fxi(), fyi = new_keyframe->fyi(),
              cxi = new_keyframe->cxi(), cyi = new_keyframe->cyi();
...
void DepthMap::regularizeDepthMap(bool removeOcclusions, int validityTH) {
  ...
  LOGF_IF(INFO, Conf().print.regularizeStatistics,
          ...
          "%d occluded; %d filled\n",
          activeKeyFrame->id(), runningStats.num_reg_smeared,
...
void DepthMap::debugPlotDepthMap(const char *buf1, const char *buf2) {
  _debugImages.debugPlotDepthMap(activeKeyFrame, currentDepthMap,
                                 referenceFrameByID_offset, buf1, buf2);
}
...
inline float DepthMap::doLineStereo(
  ...
  while (((incx < 0) == (cpx > pClose[0]) && (incy < 0) == (cpy > pClose[1])) ||
    ...
    // hacky but fast way to get error and differential error: switch buffer
  ...
  // check if interpolated error is OK. use evil hack to allow more error if
  // there is a lot of gradient.
  ...
  // * best_match_x = x-coordinate of found correspondence in the reference
  // frame.

...
Frame::Frame(int frameId, const Camera &cam, const ImageSize &sz,
    ...
    : data(frameId, timestamp, cam, sz), pose(new FramePoseStruct(*this)),
  ...
  CHECK((data.width[0] & (PYRAMID_DIVISOR - 1)) == 0)
      << "Image width " << data.width[0] << " isn't divisible by "
      << PYRAMID_DIVISOR;
  CHECK((data.height[0] & (PYRAMID_DIVISOR - 1)) == 0)
      << "Image height " << data.height[0] << " isn't divisible by "
      << PYRAMID_DIVISOR;
...
Frame::~Frame() {
  ...
  LOGF_IF(INFO, Conf().print.frameBuildDebugInfo, "DELETING frame %d",
          this->id());
...
void Frame::takeReActivationData(DepthMapPixelHypothesis *depthMap) {
  ...
  if (data.validity_reAct == 0)
    data.validity_reAct = (unsigned char *)FrameMemory::getInstance().getBuffer(
        data.width[0] * data.height[0]);

  if (data.idepth_reAct == 0)
    data.idepth_reAct = FrameMemory::getInstance().getFloatBuffer(
        (data.width[0] * data.height[0]));

  if (data.idepthVar_reAct == 0)
    data.idepthVar_reAct = FrameMemory::getInstance().getFloatBuffer(
        (data.width[0] * data.height[0]));
...
void Frame::setPermaRef(const std::unique_ptr<TrackingReference> &reference) {
  ...
  memcpy(permaRef_posData, reference->posData[QUICK_KF_CHECK_LVL],
...
void Frame::buildImage(int level) {
  ...
  if (data.image[level] == 0)
    data.image[level] = FrameMemory::getInstance().getFloatBuffer(
        data.width[level] * data.height[level]);
...
Frame::Data::Data(int i, double ts, const Camera &cam,

...
#include "viewer/PointCloudViewer.h"

#include <dynamic_reconfigure/server.h>
//#include "lsd_slam_core/LSDSLAMViewerParamsConfig.h"
...
/*
void dynConfCb(lsd_slam_core::LSDSLAMViewerParamsConfig &config, uint32_t level)
{
...
void frameCb(lsd_slam_core::keyframeMsgConstPtr msg)
{
...
void graphCb(lsd_slam_core::keyframeGraphMsgConstPtr msg)
{
	if(viewer != 0)
...
void rosThreadLoop( int argc, char** argv )
{
	...
	ROS_INFO("lsd_slam_core started");

	//dynamic_reconfigure::Server<lsd_slam_core::LSDSLAMViewerParamsConfig> srv;
	...
	rosbag::View view(bag, rosbag::TopicQuery(topics));
	 ...
	 BOOST_FOREACH(rosbag::MessageInstance const m, view)
	 {
		 ...
		 if(m.getTopic() == "/lsd_slam/liveframes" || m.getTopic() == "/lsd_slam/keyframes")
			 frameCb(m.instantiate<lsd_slam_core::keyframeMsg>());


		 if(m.getTopic() == "/lsd_slam/graph")
			 graphCb(m.instantiate<lsd_slam_core::keyframeGraphMsg>());
	 }
